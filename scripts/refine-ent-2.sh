#!/bin/bash
#SBATCH -J refine-ent-2               # (선택) Job name 변경
...
# 주기적 refine: k=1이면 매 스텝(기존 refine-ent-2와 동일), k>1이면 k 스텝마다만 2nd forward
k=8

generated_seqs_path=/home/junsu0115/remdm/outputs/refine-ent-2_T-${sampling_steps}_topp-${p}_k-${k}.json
...
python -u -m main \
    ...
    +wandb.offline=true \
    hydra.run.dir="${PWD}/outputs/refine-ent-2" \
    T=${T} \
